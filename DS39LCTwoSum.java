...
public class DS39LCTwoSum {
   ...
   public int[] twoSum(int[] numbers, int target) {
    ...
    public void push(int x) {
        if (!q1.isEmpty()) {
            q1.offer(x);  // If q1 is not empty, add to q1
        } else {
            q2.offer(x);  // If q1 is empty, add to q2
        }
    }
    ...
    public int pop() {
        if (empty()) {
            return -1;  // If both queues are empty, return -1
        }

        int top = -1;

        // Move elements from q1 to q2 until the last element
        if (!q1.isEmpty()) {
            while (!q1.isEmpty()) {
                top = q1.poll();
                if (q1.isEmpty()) {
            ...
            // Move elements from q2 to q1 until the last element
            while (!q2.isEmpty()) {
                top = q2.poll();
                if (q2.isEmpty()) {
                    break; // Found the last element
    ...
    public int top() {
        ...
        // Move elements from q1 to q2, keeping track of the last element
        if (!q1.isEmpty()) {
            while (!q1.isEmpty()) {
                top = q1.poll();
                q2.offer(top); // Add all elements to q2
                if (q1.isEmpty()) {
                    break; // Found the last element
                }
            }
            q1 = q2; //swap q1 and q2 so that top element is at the front of q1
            q2 = new LinkedList<>();
        } else {
            // Move elements from q2 to q1, keeping track of the last element
            while (!q2.isEmpty()) {
                top = q2.poll();
                q1.offer(top); // Add all elements to q1
                if (q2.isEmpty()) {
                    break; // Found the last element
                }
            }
            q2 = q1; //swap q1 and q2 so that top element is at the front of q2
            q1 = new LinkedList<>();
        }
